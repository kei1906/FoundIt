...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (user) {
      fetchConversations();
      // Added listener for profile changes to ensure live sync
  ...
  const fetchConversations = async () => {
    ...
    const mapped = chatsData.map(chat => {
      // FIX: Correctly identify the other user's ID
      ...
      return {
        ...
        // Sync profile data to the object
        otherUser: {
    ...
    <div className="min-h-screen bg-black text-white flex flex-col overflow-hidden">
      {view === 'list' ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 overflow-y-auto pb-24">
          <div className="p-6"><h1 className="text-2xl font-bold text-orange-500">Messages</h1></div>
          <div className="px-6 space-y-4">
            {conversations.map((conv) => (
              <button
                key={conv.id}
                onClick={() => selectConversation(conv)}
                className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center gap-4 text-left"
              >
                <div className="w-12 h-12 bg-white/10 rounded-full overflow-hidden shrink-0">
                  {conv.otherUser.avatar_url 
                    ...
                    : <div className="w-full h-full flex items-center justify-center"><User size={20} className="text-white/20" /></div>}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-bold truncate">{conv.otherUser.full_name}</p>
                  <p className="text-sm text-white/40 truncate">{conv.lastMessage}</p>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="flex flex-col h-screen overflow-hidden">
          {/* SYNCED HEADER */}
          <div className="flex items-center p-4 border-b border-white/5 bg-black/40 backdrop-blur-md">
            <button onClick={backToList} className="p-2 mr-2 text-orange-500"><ArrowLeft size={22} /></button>
